...
function toggle_size(e)
{
   ...
   disp.innerHTML = e.checked ? "small word set (" + lenMsg + ")" :
      "large word set (178k)";
...
function giveUp()
{
   ...
   {
      if (element.length > wordLength2 && element.length <= mainWord
         .length) //new row for next longer words:
      {
         wordLength2 = element.length;
         msg2 += "<br><span class='word-length'>" + element
            .length + "</span>";
      }
   ...
   msg3 = "<div id='missed'><i>" + wdCount + " words you missed (" + (totalLetters -
      totalLettersGot) + " letters):</i>" + msg2.substring(0, msg2.length - 2);
   ...
   { //browser can do it
      if (localStorage.getItem("highPercentage")) //exists a previous score
      {
         //new high score:
         if (percentCorrect > parseFloat(localStorage.getItem("highPercentage")))
         {
      ...
      else //first time - no high score yet - will set to zero
      {
         localStorage.setItem("highPercentage", "" + percentCorrect);
      }

      //letters:
      if (localStorage.getItem("highLetters")) //exists a previous score
      {
         //new high score:
         if (percentLetters > parseFloat(localStorage.getItem("highLetters")))
         {
...
function check2(wordToCheck)
{
   ...
   if (wordToCheck.length == 1 || wordToCheck == mainWord)
      return false;
   ...
   for (let index = 0; index < 26; index++)
   {
      if (alphaListGuess[index] > alphaListWord[index])
         return false
   }
...
function check(wordToCheck)
{
   ...
   insert2 = "<span style='color:#5CbF60'>" + wordToCheck + " is a VALID word!</span> ";

   //good word:
   if (goodLetters && inDictionary && wordToCheck != mainWord && wordToCheck.length > 1 && !
      correct_words.includes(wordToCheck))
   {
      ...
      document.getElementById("msg").innerHTML = insert + " has invalid letters";
   else if (!inDictionary)
      document.getElementById("msg").innerHTML = insert + " is not in this dictionary";
   else if (wordToCheck == mainWord)
      document.getElementById("msg").innerHTML = insert + " is the same as game word";
   ...
   if (typeof(Storage) !== "undefined")
   {
      ...
      highScoreMsg = "<span style='font-size:15px;'>YOUR BEST:<span style='margin-left:50px'> " +
         bp + "</span>% <span style='margin-left:200px'>" + bl + "</span>%</span><br>";
   }
   else
      highScoreMsg = "";


   msg += "<span id='high-scores'>" + correct_words.length +
      "<span style='font-size:24px'> words (" + percentCorrect + "%)</span>, " + totalLettersGot +
      " <span style='font-size:24px'> letters (" + percentLetters +

      "%)</span><br> " + highScoreMsg;

   //show all correct words in a list:
   wordLength = 1;
   correct_words.forEach(element =>
   {
      if (element.length > wordLength) //time for a new row
      {
         wordLength = element.length;
         msg += "<br><span style='font-size:20; color:gray;margin-right:10px;'>" +
            wordLength + "</span>";
      }
      //show most recent word in a different color:
      if (element === lastRightWord)
         msg += "<span style='color:#41b6Ff;font-size:28px;'>" + element + ", </span>";
      else
         msg += "<span style='color:white;font-size:28px'>" + element + ", </span>";
   ...
   return goodLetters && inDictionary && wordToCheck != mainWord && wordToCheck.length > 1;
}
...
function getWord()
{
   document.getElementById("word").innerHTML = "(finding word within parameters...)";
   ...
   while (!valid)
   {
      ...
      if (count >= lowerBound && count <= 200 && mainWord.length >= smallestWord && mainWord
         .length <= 13)
         valid = true;
   }
   //show counts of each word size:
   counts = document.getElementById("word").innerHTML = mainWord + " (" + count + ") </span>";
